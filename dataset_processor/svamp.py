...
def train_data():
    def format_answer(ground: str):
        ...
        return ground
    train_data = []
    data = load_dataset("/aifs4su/rubickjiang/public_data/SVAMP", split="train")
    for d in data:
        train_data.append(
            [
                {"role": "user", "content": "Question: {}.{}\nLet's think step by step:\n".format(d["Body"], d["Question"])},
...
def test_data():
    def derive_num_from_answer(answer_test):
        ...
        num = num.replace(",", "")
        return num
    data = load_dataset("/aifs4su/rubickjiang/public_data/SVAMP", split="test")
    inputs = [
        COT_EXAMPLES_chat +
        [dict(role="user", content = "Question: {}.{}\nLet's think step by step:\n".format(d["Body"], d["Question"]))]
        for d in data
